...
## 使い方
1. `HKLM` に書き込む登録/削除スクリプトは、必ず管理者権限の PowerShell で実行してください。
2. `./disable-ask-copilot-menu.ps1` を実行して項目を隠し、エクスプローラー再起動またはサインアウト/サインインで反映させます。
3. 状態を確認したいときは `./check-ask-copilot-block.ps1` を実行します。
4. 項目を復活させたい場合は `./restore-ask-copilot-menu.ps1` を実行します。
